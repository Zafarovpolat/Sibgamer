...
const AdminNotifications = () => {
  ...
  const onSubmit = async (data: NotificationForm) => {
    setIsSending(true);
    try {
      await api.post('/admin/notifications/send-to-all', {
        title: data.title,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-6">
        <Bell className="w-8 h-8 text-highlight" />
        <div>
          <h1 className="text-2xl font-bold text-white">Уведомления</h1>
          <p className="text-gray-400">Отправка уведомлений пользователям</p>
        </div>
      </div>
      ...
      <div className="glass-card p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-xl font-bold text-white">Отправить уведомление всем пользователям</h2>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-2">
              В уведомлении не добавляется текстовая подпись — пользователи увидят плашку "Сообщение от администрации"
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
            ...
            <div className="text-sm text-blue-300">
              <strong>Внимание:</strong> Это уведомление будет отправлено всем зарегистрированным пользователям сайта.
              Убедитесь, что информация важная и полезная.
            </div>
          </div>
